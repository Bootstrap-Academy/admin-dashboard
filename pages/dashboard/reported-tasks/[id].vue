<template>
	<div class="flex flex-col gap-5">
		...
		<section class="bg-[#3fdfa933] p-2 flex items-center capitalize gap-2 rounded-md">
		...
		<div class="flex flex-wrap xl:flex-nowrap justify-center">
			...
			<div class="w-2/3 p-3 flex-grow xl:flex-grow-0">
				<Reported-TasksQuizInfo :mcq="mcq" v-if="
					reportedTask.subtask_type === TASK_TYPE.MULTIPLE_CHOICE_QUESTION
				" />
				<Reported-TasksCodingChallengeInfo v-if="reportedTask.subtask_type === TASK_TYPE.CODING_CHALLENGE"
					:codingChallenge="CodingChallenge" :codingChallengeSolution="codingChallengeSolution" />
		...
		<section class="flex justify-center gap-4 mt-2 flex-wrap xl:flex-nowrap">
			<InputBtn sm :loading="loadingCorrect" @click="deleteTask(), (loadingCorrect = true)">
				{{ t("Buttons.deleteTask") }}
			</InputBtn>

			<InputBtn sm :loading="loadingCorrect" @click="
				fnResolveReport(RESOLVE.BLOCK_REPORTER), (loadingCorrect = true)
				">
				{{ t("Buttons.blockReporter") }}
			</InputBtn>

			<InputBtn sm :loading="loadingCorrect" @click="fnResolveReport(RESOLVE.BLOCK_CREATOR), (loadingCorrect = true)">
				{{ t("Buttons.blockCreator") }}
			</InputBtn>

			<InputBtn sm @click="openDialogEditTask()">
				{{ t("Buttons.adjustTask") }}
			</InputBtn>

			<InputBtn sm :loading="loadingCorrect" @click="fnResolveReport(RESOLVE.REVISE), (loadingCorrect = true)">
				{{ t("Buttons.markAsRevised") }}
			</InputBtn>

			<DialogSlot v-if="dialogEditTask" :label="'Headings.Quiz'" :show="dialogEditTask"
