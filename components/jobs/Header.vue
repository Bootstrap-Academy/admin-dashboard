<template>
	<section class="bg-secondary relative rounded shadow-xl md:col-span-2 h-fit">
		...
		<NuxtLink
			class="block absolute right-card top-card p-0"
...
export default defineComponent({
	...
	setup(props) {
		...
		const type = computed(() => {
			...
			switch (_type) {
				case 'full_time':
					return t('List.Filter.FullTime');
				case 'part_time':
					return t('List.Filter.PartTime');
				case 'internship':
					return t('List.Filter.Internship');
				case 'temporary':
					return t('List.Filter.Temporary');
				case 'mini_job':
					return t('List.Filter.MiniJob');
				default:
					return _type;
			}
		});
